...
describe('WS /live', () => {
  ...
  it('removes client on disconnect and stops receiving updates', async () => {
    ...
    expect(() => {
      store.applyEvent('yu-dev', { type: 'tool.pre', ts: 2, sessionId: 's2', agentId: 'a2', toolName: 'Read', input: {} });
    }).not.toThrow();
  });
  ...
  it('characterUpdated message includes correct character state', async () => {
    ...
    store.applyEvent('yu-dev', { type: 'tool.pre', ts: 3, sessionId: 's', agentId: 'a', toolName: 'Bash', input: {} });
    await new Promise((r) => setTimeout(r, 50));
    ...
    expect(update?.state.id).toBe('yu-dev');
  ...
  it('broadcasts to multiple connected clients simultaneously', async () => {
    ...
    store.applyEvent('han-qa', { type: 'tool.pre', ts: 4, sessionId: 's', agentId: 'a', toolName: 'Grep', input: {} });
    await new Promise((r) => setTimeout(r, 50));
