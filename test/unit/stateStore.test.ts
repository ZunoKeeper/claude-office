...
describe('stateStore', () => {
  ...
  it('agent.start → status=working, adds active ticket', () => {
    const s = createStateStore(ids);
    s.applyEvent('planner-researcher', { type: 'agent.start', ts: 1, sessionId: 's', agentType: 'Explore', agentId: 'a1' }, 'find kafka');
    const st = s.get('planner-researcher');
  ...
  it('second agent.start on same char → queued ticket', () => {
    const s = createStateStore(ids);
    s.applyEvent('planner-researcher', { type: 'agent.start', ts: 1, sessionId: 's', agentType: 'Explore', agentId: 'a1' }, 'x');
    s.applyEvent('planner-researcher', { type: 'agent.start', ts: 2, sessionId: 's', agentType: 'Explore', agentId: 'a2' }, 'y');
    expect(s.get('planner-researcher').queue.map((t) => t.status)).toEqual(['active', 'queued']);
  });

  it('agent.stop removes active ticket and promotes next', () => {
    const s = createStateStore(ids);
    s.applyEvent('planner-researcher', { type: 'agent.start', ts: 1, sessionId: 's', agentType: 'Explore', agentId: 'a1' });
    s.applyEvent('planner-researcher', { type: 'agent.start', ts: 2, sessionId: 's', agentType: 'Explore', agentId: 'a2' });
    s.applyEvent('planner-researcher', { type: 'agent.stop', ts: 3, sessionId: 's', agentId: 'a1', success: true });
    const st = s.get('planner-researcher');
    expect(st.queue).toHaveLength(1);
  ...
  it('empty queue after final stop → status=done, stats increment', () => {
    const s = createStateStore(ids);
    s.applyEvent('planner-researcher', { type: 'agent.start', ts: 1, sessionId: 's', agentType: 'Explore', agentId: 'a1' });
    s.applyEvent('planner-researcher', { type: 'agent.stop', ts: 2, sessionId: 's', agentId: 'a1', success: true });
    const st = s.get('planner-researcher');
