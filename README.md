...
- 상단 [GRID | OFFICE] 토글
  ...
  - **OFFICE**: PixiJS 사무실 씬 — 그리드와 동일한 픽셀 캐릭터가 실제 사무실 인테리어
    (회의실·개발실·서버실·서고·검수/디자인·탕비실) 안에서 앉아있고, 툴 실행 시
    해당 공간으로 걸어감. 걷는 방향에 따라 스프라이트가 좌우로 반전됨.
    박PL이 dispatch 라인을 낼 때마다 대상 방향으로 반쯤 걸어갔다 자리로 복귀.
...
- **오피스 뷰 상호작용 확장**: 현재는 툴 이동 + PL dispatch walk만. 후속: 캐릭터 간 대화 애니, 커피 마시러 탕비실 방문, 프린터 왕복, 좌우 걸음 프레임 애니(현재 static bob) 등.
